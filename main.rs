...
fn genrandomnumarr(lownum : u32 , highnum : u32) -> Vec<u32> {
    ...
    while i < highnum {
        ...
        if randnum != i.try_into().unwrap() {
            randarr.swap(i.try_into().unwrap(),<u32 as TryInto<usize>>::try_into(randnum).unwrap());
        }
        i = i + 1; 
    };
...
fn startquiz(questions : Vec<&str> , awnsers : Vec<&str>) {
    ...
    for x in quizorderarr.iter() {
        let v = *x as usize;
        slow_print(format!("\n{}\n",questions[v]).as_str(),STANDARDSLOWPRINTTIME );
        ...
        slow_print(format!("\nCorrect awnser: -> {}\n",awnsers[v]).as_str(),STANDARDSLOWPRINTTIME);
        slow_print("--------------------\n",STANDARDSLOWPRINTTIME);
        usr_input("");
    }
...
fn list_quizfiles() -> Result<Vec<std::path::PathBuf> , io::Error> {
    Ok(fs::read_dir("./")?
...
fn selectquizfile() -> String {
    let availablequizez = list_quizfiles().unwrap();
    if availablequizez.len() == 0 {
        ...
        while selectedquiz.len() == 0 {
            ...
            for (i,q) in availablequizez.iter().enumerate() {
                slow_print(format!("{}.) {}\n" ,i, q.as_path().display()).as_str(),STANDARDSLOWPRINTTIME);
            }

            let selectval = usr_input("Select available quiz by number -> ").trim().parse::<u32>();
            match selectval {
                Ok(v)  =>{
                    if v < availablequizez.len().try_into().unwrap() {
                        selectedquiz = availablequizez[<u32 as TryInto<usize>>::try_into(v).unwrap()].as_path().to_str().unwrap();
                    }
                    else{
...
fn main() {
    ...
    let quizfilepath;
    if args.len() == 1 {
        //println!("No quiz file provided");
        quizfilepath = selectquizfile();
        if quizfilepath.len() == 0 {
            return;
        }
    }
    else {
        quizfilepath = args[1].to_string();
    }
